...
| Capability | Arch ([`packages.txt`](../packages.txt)) | Mint (`apt`, [`mint-apt-cli.txt`](../linux-mint/mint-apt-cli.txt)) | Otherwise |
|------------|--------------------------------------------|---------------------------------------------------------------|-----------|
| Shell | `zsh` | `zsh` | — |
| Prompt (zsh) | Powerlevel10k (git; [`arch/bootstrap.sh`](../arch/bootstrap.sh)) | same clone step | — |
| Prompt (bash) | optional Starship | optional Starship | [`linux-mint/bootstrap.sh`](../linux-mint/bootstrap.sh) |
...
| Dotfile manager | `chezmoi` | — | official install ([bootstrap.sh](../linux-mint/bootstrap.sh)) |
| Python tooling | `uv` | — | Astral install script |
| Obsidian | `obsidian` (pacman) | Flatpak ([`run_once_install_flatpaks.sh`](../linux-mint/run_once_install_flatpaks.sh)) | intentional difference |
...
- **Bootstrap**: [`macos/install.sh`](../macos/install.sh) runs `brew bundle`, [`scripts/install-omz-p10k.sh`](../scripts/install-omz-p10k.sh), Cursor/VS Code extensions; **chezmoi** is the canonical dotfile path—run `chezmoi apply` with this repo as your source (see [README](../README.md)). Post-setup: [`macos/verify.sh`](../macos/verify.sh).
- **Window tiling**: **Rectangle** ([`macos/Brewfile`](../macos/Brewfile) cask `rectangle`) — not Slate. Configure shortcuts in Rectangle → Settings after install. Linux XFCE/Mint uses **Super+Left/Right** via wmctrl scripts ([`linux-mint/xfce-ux-mint.sh`](../linux-mint/xfce-ux-mint.sh)); on macOS use Rectangle’s defaults (e.g. **Ctrl+Option+arrow** for halves) or your own bindings.
...
- **Bootstrap**: [`linux-mint/bootstrap.sh`](../linux-mint/bootstrap.sh) — APT packages, **chezmoi**, **uv**, **Starship**, Oh My Zsh, Powerlevel10k. Use `--laptop` for TLP/powertop ([`linux-mint/mint-apt-laptop.txt`](../linux-mint/mint-apt-laptop.txt)). Lighter alternative: [`linux-mint/mint-bootstrap.sh`](../linux-mint/mint-bootstrap.sh) (APT + OMZ/p10k + `chezmoi apply` only).
- **APT list**: [`linux-mint/mint-apt-cli.txt`](../linux-mint/mint-apt-cli.txt) — same packages as copy-paste reference.
...
- **Script**: [`linux-mint/xfce-ux-mint.sh`](../linux-mint/xfce-ux-mint.sh) — run inside a graphical session (`DISPLAY` set). Installs APT packages from [`linux-mint/mint-apt-xfce-ux.txt`](../linux-mint/mint-apt-xfce-ux.txt), sets **Kitty** as XFCE default terminal (`exo-preferred-applications`), then applies xfconf (Arc-Dark / Papirus / Bibata cursor, `tile_on_move`, xfwm4 shortcuts). Flags: `--apt-only`, `--xfconf-only`.
...
- **Tiling scripts**: [`linux-mint/bin/xfce-tile-left.sh`](../linux-mint/bin/xfce-tile-left.sh), [`linux-mint/bin/xfce-tile-right.sh`](../linux-mint/bin/xfce-tile-right.sh), [`linux-mint/bin/xfce-window-maximize.sh`](../linux-mint/bin/xfce-window-maximize.sh), [`linux-mint/bin/xfce-window-restore.sh`](../linux-mint/bin/xfce-window-restore.sh) — installed to `~/.local/bin/` by [`linux-mint/xfce-ux-mint.sh`](../linux-mint/xfce-ux-mint.sh). **Super+Up** maximizes; **Super+Down** restores default size. Requires **wmctrl** ([`linux-mint/mint-apt-xfce-ux.txt`](../linux-mint/mint-apt-xfce-ux.txt)).
- **macOS parity**: No XFCE automation—terminal/font story stays **iTerm2** + Nerd Font from [`macos/Brewfile`](../macos/Brewfile); this block is Linux-only.
...
- **macOS**: Cask `font-jetbrains-mono-nerd-font` in [`macos/Brewfile`](../macos/Brewfile). In **iTerm2**: Settings → Profiles → Text → **Font** → choose **JetBrainsMono Nerd Font** (or **NF**).
...
```bash
bash linux-mint/install-nerd-font-jetbrains.sh
```
...
```bash
# Linux Mint
bash linux-mint/verify.sh

# macOS
bash macos/verify.sh

# Fedora Kinoite
bash fedora-kinoite/verify.sh
```
...
- Install [Oh My Zsh](https://ohmyz.sh/) and clone Powerlevel10k if [`linux-mint/bootstrap.sh`](../linux-mint/bootstrap.sh), [`fedora-kinoite/bootstrap.sh`](../fedora-kinoite/bootstrap.sh), [`macos/install.sh`](../macos/install.sh), or [`arch/bootstrap.sh`](../arch/bootstrap.sh) has not already done so.
