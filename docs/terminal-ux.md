...
- **Window tiling**: **Rectangle** ([`Brewfile`](../Brewfile) cask `rectangle`) — not Slate. Configure shortcuts in Rectangle → Settings after install. Linux XFCE parity uses xfwm4 Super+arrow keys ([`linux/xfce-ux-mint.sh`](../linux/xfce-ux-mint.sh)); on macOS use Rectangle’s defaults (e.g. **Ctrl+Option+arrow** for halves) or your own bindings.
...
- **Whisker Menu (required)**: On X11, **Whisker Menu must not use Super alone** — it blocks **Super+arrow** from reaching xfwm4. Bind Whisker to **Super+Space** or **Alt+F1** before relying on tiling keys.
